// Package hmacsha512 contains the libsodium bindings for HMAC-SHA512.
package hmacsha512
...
const (
	Bytes    int = C.crypto_auth_hmacsha512_BYTES
	KeyBytes int = C.crypto_auth_hmacsha512_KEYBYTES
)
...
func CheckMAC(in []byte, h *[Bytes]byte, key *[KeyBytes]byte) (err error) {
	...
	exit := C.crypto_auth_hmacsha512_verify(
		(*C.uchar)(&h[0]),
		(*C.uchar)(support.BytePointer(in)),
		(C.ulonglong)(len(in)),
		(*C.uchar)(&key[0]))
